...
include { BAM_STATS_ONLY_SAMTOOLS } from '../../local/bam_index_stats_samtools'
...
workflow ALIGN_STAR {
    ...
    if (is_aws_igenome) {
        ...
        // Bedgraph not available in igenomes version, will remain empty
        ...
        ch_bedgraph       = STAR_ALIGN.out.bedgraph  // Capture bedgraph from STAR output
    ...
    //
    BAM_STATS_ONLY_SAMTOOLS ( ch_orig_bam, fasta )
    ch_versions = ch_versions.mix(BAM_STATS_ONLY_SAMTOOLS.out.versions)
    ...
    bam            = BAM_STATS_ONLY_SAMTOOLS.out.bam      // channel: [ val(meta), [ bam ] ]
    bai            = BAM_STATS_ONLY_SAMTOOLS.out.bai      // channel: [ val(meta), [ bai ] ]
    csi            = BAM_STATS_ONLY_SAMTOOLS.out.csi      // channel: [ val(meta), [ csi ] ]
    stats          = BAM_STATS_ONLY_SAMTOOLS.out.stats    // channel: [ val(meta), [ stats ] ]
    flagstat       = BAM_STATS_ONLY_SAMTOOLS.out.flagstat // channel: [ val(meta), [ flagstat ] ]
    idxstats       = BAM_STATS_ONLY_SAMTOOLS.out.idxstats // channel: [ val(meta), [ idxstats ] ]
