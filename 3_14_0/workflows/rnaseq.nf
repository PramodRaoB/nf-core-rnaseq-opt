...
include { SAMTOOLS_PRESORT_QUALIMAP } from '../modules/local/samtools_presort_qualimap'
...
include { BAM_STATS_ONLY_SAMTOOLS          } from '../subworkflows/local/bam_index_stats_samtools'
...
workflow RNASEQ {
    ...
    ch_star_bedgraph              = Channel.empty() // Initialize channel for STAR bedgraph output
    ...
    if (!params.skip_alignment && params.aligner == 'star_salmon') {
        ...
        ch_star_bedgraph     = ALIGN_STAR.out.bedgraph  // Capture bedgraph output from STAR
    ...
    if (!params.skip_alignment && !params.skip_markduplicates && !params.with_umi) {
        // Only pass bedgraph channel if we're using STAR
        ch_bedgraph_for_picard = params.aligner.contains('star') ? ch_star_bedgraph : Channel.empty()

        BAM_MARKDUPLICATES_PICARD (
            ch_genome_bam.join(ch_genome_bam_index, by: [0]),
            PREPARE_GENOME.out.fasta.map { [ [:], it ] },
            PREPARE_GENOME.out.fai.map { [ [:], it ] },
            ch_bedgraph_for_picard
        )
    ...
    if (!params.skip_alignment && !params.skip_qc) {
        if (!params.skip_qualimap) {
            try {
                SAMTOOLS_PRESORT_QUALIMAP ( ch_genome_bam )
                QUALIMAP_RNASEQ (
                    SAMTOOLS_PRESORT_QUALIMAP.out.bam,
                    PREPARE_GENOME.out.gtf.map { [ [:], it ] }
                )
                ch_qualimap_multiqc = QUALIMAP_RNASEQ.out.results

                // Add debugging for versions collection
                QUALIMAP_RNASEQ.out.versions.first().view { "Qualimap versions: $it" }
                SAMTOOLS_PRESORT_QUALIMAP.out.versions.first().view { "Samtools versions: $it" }

                //ch_versions = ch_versions.mix(QUALIMAP_RNASEQ.out.versions.first())
                //ch_versions = ch_versions.mix(SAMTOOLS_PRESORT_QUALIMAP.out.versions.first())
            } catch (Exception e) {
                log.error "Error in Qualimap section: ${e.message}"
                throw e
            }
        }
